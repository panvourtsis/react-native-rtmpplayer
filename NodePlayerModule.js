...
import { requireNativeComponent, View, UIManager, findNodeHandle, NativeModules } from 'react-native';
...
class NodePlayerView extends Component {
  ...
  seekTo(position) {
    NativeModules.NodeMediaClient.seekTo(position);
  }

  getDuration(callback) {
    NativeModules.NodeMediaClient.getDuration(callback);
  }

  getCurrentPosition(callback) {
    NativeModules.NodeMediaClient.getCurrentPosition(callback);
  }

  getBufferPosition(callback) {
    NativeModules.NodeMediaClient.getBufferPosition(callback);
  }

  getBufferPercentage(callback) {
    NativeModules.NodeMediaClient.getBufferPercentage(callback);
  }

  isPlaying(callback) {
    NativeModules.NodeMediaClient.isPlaying(callback);
  }

  isLive(callback) {
    NativeModules.NodeMediaClient.isLive(callback);
  }
